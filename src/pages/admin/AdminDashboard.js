...
const AdminDashboard = ({ admin, onLogout }) => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load stats
      const statsResponse = await fetch('https://ai-dashboard-task-backend-1.onrender.com/api/admin/stats', {
      ...
      // Load users
      const usersResponse = await fetch('https://ai-dashboard-task-backend-1.onrender.com/api/admin/users?limit=100', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      // Load chats
      const chatsResponse = await fetch('https://ai-dashboard-task-backend-1.onrender.com/api/admin/chats?limit=50', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`https://ai-dashboard-task-backend-1.onrender.com/api/admin/users/${userId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDeleteChat = async (chatId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`https://ai-dashboard-task-backend-1.onrender.com/api/admin/chats/${chatId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
